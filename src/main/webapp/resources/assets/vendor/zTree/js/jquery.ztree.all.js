  ...
  var settings = {}, roots = {}, caches = {},
    ...
    view = {
      ...
      makeDOMNodeIcon: function (html, setting, node) {
        ...
        if(node.deptCode === "user") {
          html.push("<span id='", node.tId, consts.id.ICON,
              "' title='' treeNode", consts.id.ICON, " class='", view.makeNodeIcoClass(setting, node), " tree-node-user' data-id='", node.id,
      ...
      makeNodeIcoClass: function (setting, node) {
        ...
        if (!node.isAjaxing) {
          ...
          if (isParent) {
            ...
            if(node.parent) {
              if(node.enabled != null && !node.enabled) {
                icoCss.push("docud");
              }
              else {
                if(node.deptCode) {
                  ...
                  icoCss.push(consts.folder.CATE);
                }
              }
            }
            else {
      ...
      addHoverDom: function (setting, node) {
        if (data.getRoots().showHoverDom) {
          ...
          if (setting.edit.enable) {
            ...
            //if(!node.isParent || node.children == null || node.children.length ==0) {
